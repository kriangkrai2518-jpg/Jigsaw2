...
st.title("🎬 Jigsaw Master (Per-Scene Voiceover)")
...
with col1:
    ...
    st.info("💡 คุณสามารถเพิ่มเสียงพากย์แยกแต่ละฉากได้ในส่วนจัดการด้านล่าง")
    global_bgm = st.file_uploader("Upload Background Music (BGM)", type=["mp3"])
...
if uploaded_files:
    ...
    for i, file in enumerate(sorted_files):
        with st.expander(f"🎤 Scene {i+1}: {file.name}", expanded=True):
            ...
            with sc_col2:
                # ส่วนเพิ่มใหม่: ตัวเลือกอัปโหลดเสียงพากย์ราย Scene
                v_file = st.file_uploader(f"Add Voiceover", type=['mp3', 'wav'], key=f"voice_{i}")
                dur = st.slider(f"Duration", 1.0, 20.0, get_reading_duration(cap), key=f"dur_{i}")
            
            scene_configs.append({"file": file, "cap": cap, "dur": dur, "voice": v_file})

    if st.button("🚀 Start Render Final Video"):
        with st.status("🎬 Processing with Per-Scene Voiceover...") as status:
            try:
                final_clips = []
                for config in scene_configs:
                    # บันทึกไฟล์ชั่วคราวสำหรับภาพ/วิดีโอ
                    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(config["file"].name)[1]) as t:
                        t.write(config["file"].getvalue())
                        temp_path = t.name

                    # ประมวลผลภาพ/วิดีโอพื้นฐาน
                    if config["file"].name.lower().endswith('.mp4'):
                    ...
                    # --- ส่วนจัดการเสียงพากย์ราย Scene ---
                    if config["voice"]:
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as v_temp:
                            v_temp.write(config["voice"].getvalue())
                            v_audio = AudioFileClip(v_temp.name).volumex(voice_volume)
                            # ปรับความยาวคลิปให้เท่ากับเสียงพากย์ (ถ้าต้องการ)
                            # clip = clip.set_duration(v_audio.duration) 
                ...
                # ใส่เสียง BGM คลอเบาๆ ตลอดทั้งคลิป
                if global_bgm:
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as bg_temp:
                        bg_temp.write(global_bgm.getvalue())
                        bg_audio = AudioFileClip(bg_temp.name).volumex(bgm_volume).set_duration(full_video.duration)
                        # ผสมเสียง BGM กับเสียงพากย์ที่มีอยู่แล้ว
                ...
                out_file = "jigsaw_voiceover_final.mp4"
                ...
                status.update(label="✅ Render สำเร็จพร้อมเสียงพากย์!", state="complete")
